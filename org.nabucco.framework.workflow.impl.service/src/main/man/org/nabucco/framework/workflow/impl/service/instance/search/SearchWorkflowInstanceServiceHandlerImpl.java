...
/**
 * SearchWorkflowInstanceServiceHandlerImpl
 * 
 * @author Nicolas Moser, PRODYNA AG
 */
public class SearchWorkflowInstanceServiceHandlerImpl extends SearchWorkflowInstanceServiceHandler {

    private static final long serialVersionUID = 1L;

    @Override
    public WorkflowInstanceListMsg searchWorkflowInstance(WorkflowInstanceSearchRq rq) throws SearchException {

        WorkflowInstanceListMsg rs = new WorkflowInstanceListMsg();

        WorkflowInstanceSearchParameter param = rq.getSearchParameter();

        try {
            List<WorkflowInstance> resultList = this.executeQuery(param);
            rs.getWorkflowInstanceList().addAll(resultList);

        } catch (Exception e) {
            throw new SearchException("Error searching for Workflow Instance.", e);
        }
     ...
     */
    private List<WorkflowInstance> executeQuery(WorkflowInstanceSearchParameter wisp) throws PersistenceException {

        List<WorkflowInstance> instances = new ArrayList<WorkflowInstance>();

        if (wisp.getDefinitionParameters().isEmpty()) {
            NabuccoQuery<WorkflowInstance> query = this.createQuery(wisp, null);
        ...
        for (WorkflowDefinitionSearchParameter wdsp : wisp.getDefinitionParameters()) {
            ...
            NabuccoQuery<WorkflowInstance> query = this.createQuery(wisp, wdsp);
            this.addInstanceParameters(query, wisp);
     ...
     */
    private NabuccoQuery<WorkflowInstance> createQuery(WorkflowInstanceSearchParameter wisp,
            WorkflowDefinitionSearchParameter wdsp) throws PersistenceException {

        StringBuilder queryString = new StringBuilder();

        queryString.append("select i from WorkflowInstance i");
        queryString.append(" inner join i.currentEntry e");
        queryString.append(" inner join i.definition d");
        queryString.append(" inner join e.state s");

        // Instance

        queryString.append(" where (i.owner = :owner or :owner is null)");
        queryString.append(" and (i.name = :name or :name is null)");
        queryString.append(" and (i.summary like :summary or :summary is null)");
     ...
     */
    private void addInstanceParameters(NabuccoQuery<WorkflowInstance> query, WorkflowInstanceSearchParameter param)
            throws PersistenceException {
     ...
     *             when the parameters cannot be set appropriately
     */
    private void addDefinitionParameters(NabuccoQuery<WorkflowInstance> query, WorkflowDefinitionSearchParameter wdsp)
            throws PersistenceException {
     ...
     *             when the parameters cannot be set appropriately
     */
    private void addStateParameter(NabuccoQuery<WorkflowInstance> query, WorkflowDefinitionSearchParameter wdsp)
            throws PersistenceException {
